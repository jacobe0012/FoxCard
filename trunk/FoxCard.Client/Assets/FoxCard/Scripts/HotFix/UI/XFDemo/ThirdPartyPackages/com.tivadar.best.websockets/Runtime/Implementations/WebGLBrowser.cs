...
namespace Best.WebSockets.Implementations
{
    ...
    internal sealed class WebGLBrowser : WebSocketBaseImplementation
    {
        ...
        public override void StartOpen()
        {
            try
            {
                string uriStr = this.Uri.OriginalString;
                // int questionMarkIndex = uriStr.IndexOf('?');
                //
                // if (questionMarkIndex != -1 && uriStr[questionMarkIndex + 1] == '&')
                // {
                //
                //     uriStr = uriStr.Remove(questionMarkIndex + 1, 1);
                // }
                
                ImplementationId = WS_Create(uriStr, this.Protocol, OnOpenCallback, OnTextCallback, OnBinaryCallback, OnErrorCallback, OnCloseCallback, Allocator);
